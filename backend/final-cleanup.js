...
async function finalCleanup() {
  ...
  if (remainingIssues.length === 0) {
    ...
    remainingIssues.forEach(product => {
      console.log(`   - ${product.name}: seller=${product.seller?._id}, firstName=${product.seller?.firstName}, lastName=${product.seller?.lastName}, artisan=${product.artisan?._id}, artisanName=${product.artisan?.artisanName}`);
    });
  }
