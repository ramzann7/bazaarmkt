...
async function setupProductionDatabase() {
  ...
  try {
    ...
    console.log('\n🔧 Creating essential collections...');
    for (const collectionName of essentialCollections) {
      ...
      if (existingCollections.length > 0) {
        ...
        console.log(`📄 ${collectionName} has ${docCount} documents`);
      } else {
      ...
      // Set up indexes
      await setupCollectionIndexes(db, collectionName);
    }
    ...
    if (error.code === 8000) {
      ...
      console.log('4. Make sure you have a separate production cluster set up');
    }
  } finally {
    // Close connection
    if (client) {
